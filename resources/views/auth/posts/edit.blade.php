...
@section('title','Edit Post | Admin Dashbord');
            ...
            <div class="card-body">
                ...
                <form method="POST" action="{{ route('posts.update', $post->id) }}" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="description" class="form-control" cols="30" rows="3" style="resize: none">{{ old('description', $post->description) }}</textarea>
                    </div>
                        ...
                        <select name="tags[]" class="form-control selectpicker" multiple data-live-search="true">
                            ...
                            @foreach ($tags as $tag)
                                @php
                                    ...
                                    $isSelected = $post->tags->contains($tag->id) || in_array($tag->id, old('tags', []));
                                @endphp
                                <option value="{{ $tag->id }}" @selected($isSelected)>{{ $tag->name }}</option>
                            @endforeach
                        </select>
                    </div>

                    <div class="form-group md-4">
                        <label for="fileInput"><i class="fas fa-camera"></i> Image</label>
